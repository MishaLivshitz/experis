...
/* template<typename T>
inline typename SortedVector<T>::const_iterator SortedVector<T>::cbegin() const
{
    return Base::begin();
}

template<typename T>
inline typename SortedVector<T>::const_iterator SortedVector<T>::cend() const
{
    return Base::end();
} */
...
inline void SortedVector<T>::erase(const_iterator a_begin, const_iterator a_end) 
{
    ...
    iterator b = begin();
    iterator e = begin();
    b += std::distance(Base::cbegin(),a_begin);
    e += std::distance(Base::cbegin(),a_end);
    Base::erase(b,e);
