...
//	*	mem-function:	front		rturn front of the queue without extracting
//	*	mem-function:	isFull()
...
public:
	//CTOR
	RingBuffer(size_t _capacity = 64);
	//CCTOR
	RingBuffer(RingBuffer<T> const& _source);
	//DTOR
	~RingBuffer();
	//Assignment operator
	RingBuffer& operator= (RingBuffer<T> const& _source);
	
	void enqueue(const T& _value);
	void dequeue(T& _value);
...
//CTOR
template <typename T>
RingBuffer<T>::RingBuffer(size_t _capacity)
...
//Assignment operator
template <typename T>
RingBuffer<T>& RingBuffer<T>::operator= (RingBuffer<T> const& _source)
{
	//What if the capacity of this is smaller?
	if(this != &_source && m_capacity >= _source.m_size)
	{	
		init(_source);
	}
...
//public funcs
template<typename T>
inline void RingBuffer<T>::enqueue(const T& _value)
{
	if(!isFull())
	{
...
template<typename T>
inline void RingBuffer<T>::dequeue(T& _value)
{
	if(!isEmpty())
	{
