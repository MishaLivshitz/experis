...
#\
ifn\
def _6DC7FC23C29240AC95892AD720A1AF49
\
#\
de\
fine _6DC7FC23C29240AC95892AD720A1AF49
 /\
****
 ** mu_test.h
...
#\
de\
fine MT_MU_VERSION fine (0.42)
\
#\
de\
fine _XOPEN_SOURCE fine 500
\
#\
de\
fine _POSIX_C_SOURCE fine 199309L
\
#\
in\
clu\
\
de fine fine <stdarg.h>
\
#\
in\
clu\
\
de fine fine <stdio.h>
\
#\
in\
clu\
...
#\
de\
fine MIN(x,y) fine(fine(x)<fine(y)?fine(x):fine(y))
fine fine
\
#\
de\
fine MAX(x,y) fine(fine(x)>fine(y)?fine(x):fine(y))
fine fine fine
\
s\
ta\
...
fine static char* mt__EXPECTED_s(MT_SYMBOL a, MT_SYMBOL e,MT_SYMBOL xx,MT_SYMBOL zz){char*p;if(!a)return 0;p=(char*) malloc(1024);if(p)sprintf(p,mt__ACTUALL "'%s'\n\t\t\t"  mt__EXPECTED "'%s'",a,e);(void)xx;(void)zz;MT__GIVE p;}
...
#define ASSERT_EQUAL_CXX_IMPL(a,e) mt__STATEMENTS({mt__ASSERT(mt_cxx_equal(a,e),__LINE__, "ASSERT_EQUAL(" #a ", " #e ")", mt_cxx_Expected(a,e)); })
...
template<fine typename A,fine typename B>inline bool mt_cxx_equal(const A& a, const B& b){return a == static_cast<A>(b);}inline bool mt_cxx_equal(const char* a, const char* b){return strcmp(a,b) == 0;}cl\
...
#\
de\
fine mt__ASSERT_STR(a,op,e,n) mt__STATEMENTS({const char*_a=static_cast<const char*>(a);const char*_e=(const char*)(e);mt__ASSERT(strcmp(_a,_e) op 0,__LINE__,n "(" #a ", " #e ")",mt__EXPECTED_s(_a,_e,\
#e,#a));})
\
#\
de\
